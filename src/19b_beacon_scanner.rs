...
fn main() {
    ...
    let mut coord_set: Vec<BTreeSet<P3>> = vec![BTreeSet::new(); scanners.len()];
    for index in visited.iter().rev() {
        ...
        coord_set[*index].insert(P3(0, 0, 0));
        let translated: Vec<_> = coord_set[*index]
            .iter()
            .map(|point| &rotate(point) - &translation)
            .collect();

        coord_set[relative_to].extend(translated.into_iter());
    }

    let mut max_dist = 0;
    for a in coord_set[0].iter() {
        for b in coord_set[0].iter() {
            let diff = a - b;
            max_dist = cmp::max(max_dist, diff.0.abs() + diff.1.abs() + diff.2.abs())
        }
    }
    println!("{}", max_dist);
}
